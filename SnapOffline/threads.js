...
Process.prototype.reportABCmd = function (cmd) {

	var response = 'waiting...';

    // The ID of the extension we want to talk to.
    var editorExtensionId = "bflaggopagnplpmhoeeddhofaopeepfd";
	console.log('input cmd: ' + cmd);
	
	if(cmd != 'checkState'){
		// Make a request:
		chrome.runtime.sendMessage(editorExtensionId, {command: cmd},
		function(responseFromExtension) {
			response = "feedback: " + responseFromExtension.feedback;
		...
		chrome.runtime.sendMessage(editorExtensionId, {command: cmd},
		function(responseFromExtension) {
			state = responseFromExtension.feedback;
			console.log('previous cmd processing state : ' + state);
		});
	}
	/*
	var doSuccess = function(){
		if(response!='waiting...'){
			...
			waiTimer = setTimeout(doSuccess,100);
		}
	}
